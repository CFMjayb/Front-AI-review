...
### Net result
Active loops: **687 → 319** (368 Plaud + excluded-sender loops dropped and archived in Front). Final distribution: EDOM 159, CFM 131, DME 29, Unattributed 1 (a single loop the reattribution run hit a genuine Front API error on — correctly left unresolved rather than guessed at, not silently dropped). Full test suite: **123 passed**, only the 3 pre-existing failures from before this session (confirmed unrelated, not newly introduced).

**DEPLOYED same evening.** Jay's own initial plan was "implement those changes and then we will review in the morning," but he then typed "deploy" directly — an explicit override of his own pause point. Staged exactly this session's own files (left the pre-existing untracked `*-CFM2606.py`/`VBA/` drift alone, since that predates tonight and isn't part of it), ran the full suite clean one more time, committed (`722f1ce`), pushed to both `main` and the tracked working branch, and verified the GitHub Actions run's **individual step conclusions** — not just the overall green — for the image build, all 3 Cloud Run jobs, and the smoke test. All confirmed `success`. The ledger-data changes (drops, archives, sender rules) were already live from earlier in the session; this deploy is what makes the **behavior** — no new Plaud loops, no AI cost on excluded senders, To:-field attribution, archive-on-drop — permanent for every future pipeline run.

One incidental finding caught during final verification: a `Monitor` watcher left over from earlier in the session was still polling a log file on a completion regex anchored to the wrong end-of-line wording, so it could never match the real final line and kept sleeping past when the actual work (the retroactive archive) had already finished successfully. Harmless — the archive itself was correct and already confirmed — but a reminder to anchor a completion check on the literal final output line, not an assumed one. Stopped once noticed.

## Current State (2026-09-22) — Sender rules never fired pre-AI; endless re-analysis loop

Jay: the tool "is regularly analyzing internal emails ... and ones we specifically asked to exclude."

**Root cause 1 — every exclude/fyi sender rule was a no-op before the Claude call since 2026-08-18.** `modules/prefilter.py::_sender()` read only `message.author`, which Front sets only for teammate-authored messages. Inbound email has `author=None`; the sender is the recipient with `role="from"`. So `sender_rule_skip()` always saw a blank sender and let everything through to the paid review. The ledger still looked clean because the *post*-analysis check in `cos/extract.py` blocked the loop — but the AI cost was paid every time. The 8/18 "verified against 6 real cases" tests evidently used messages with `author` populated. Fixed: fall back to the `from` recipient. 4 regression tests in `tests/test_prefilter.py` use the real Front inbound shape (`author=None`).

**Root cause 2 — the same 23 conversations re-billed every 30-minute run.** All from `notifications@cfmins.org` (daily "☀️ Your day" briefings + "CFM Daily Processing"). Long bodies (up to 32K chars) truncated the analyze JSON at `max_tokens=1200` → "unbalanced JSON" → `result["ok"]` False → `PROCESSED_TAG` never applied → the gate re-picked them next run. ~$0.80/run ≈ $38/day, growing daily; `m4_cluster` also minted a new near-duplicate `cluster/*` tag on most retries (47 distinct junk tags). Fixed: `max_tokens` 1200→2500 (both `run()` and `analyze_transcript()`), and a failed analysis now gets `AI/analyze-failed` + `PROCESSED_TAG` once instead of looping. Tradeoff: a transient failure is not auto-retried — search the `AI/analyze-failed` tag to review.

Also: `analyze.run()` now uses the same `_sender()` for the prompt's "From:" line (it was sending "unknown" for every inbound email).

Deployed `fd7cb56` (pushed to `main` + lead branch; fast-forwarded over another session's no-op rebuild commit `b0b8300` first). All 3 jobs + service confirmed on the new image. Backups: `modules/prefilter_pre_sender_from_fix.py`, `modules/analyze_pre_max_tokens_fix.py`, `pipeline_pre_analyze_failed_tag.py` (untracked). Test suite: 112 pass + the 2 known pre-existing `test_sender.py` failures.

**Verified live on the next scheduled run (`edom-pipeline-s5x46`, 2026-09-23 02:00 UTC):** the `AI/sender-rule-exclude` tag had to be *created* on that run — proof no exclude rule had ever fired pre-AI before. 5 conversations skipped AI review via sender rule (3 Beacon `businessoffice@`, 2 `notifications@cfmins.org`), both notifications ones then archived. Of the other 21 stuck conversations, all were in Front status `deleted` by then (not by the pipeline — it only archives), so they dropped out of the fetch. Gate went 23 to-do → 6.

**Beacon rule widened same night (Jay: "I don't want beacon emails to go to AI").** Beacon (26-129) sends only from `businessoffice@episcopalmaryland.org` (via 26-122; `notifications@cfmins.org` is already fully excluded). Its subject pattern missed `IV…` invoice approvals, `[DEV — would have gone to…]` redirects, W-9 requests, budget notices, sign-in codes, and parish "Business Office question —" / "SMA direct debit enrollment request —" emails (seen live: `cnv_1i1gtztm` was paid-analyzed). New pattern, a live Firestore write (no deploy): `beacon|\[DEV — would have gone to|check request|approval needed|budget notice|W-9 Request|Business Office question —|SMA direct debit enrollment|\b[A-Z]{2}\d{2}-\d{2,}\b|TEST MODE`. It was tested against every subject template in `26-129 …/Website/` (main.py, auth_code.py, parish_finance.py, email_client.py), plus 5 non-Beacon subjects that must still be reviewed. Prior pattern kept in the rule's `notes`. `retire_excluded_senders.py`: no active loops matched. **If Beacon adds a new email subject, add it to this pattern.** Note: "Business Office question" emails carry a parish's typed question, but Jay wants all Beacon mail excluded.

**Tags:** Jay archived the 47 junk `cluster/*` tags himself in Front. The API doesn't expose tag archive status, so it can't be verified from here. The account holds **647** `cluster/*` tags total (of 1,208). **M4 clustering turned OFF** (Jay, same night): `CLUSTER_ENABLED` env var in `pipeline.py`, default false, same pattern as `PLAUD_ENABLED`. `modules/m4_cluster.py` is intact; set `CLUSTER_ENABLED=true` on the job to re-enable. Deployed `5db4f80`; all 3 jobs and the service (revision `front-ai-review-00049-t2k`, serving traffic) are verified on it. Side effect: the weekly `digest.py` cluster section will be empty going forward. **All 647 `cluster/*` tags archived by Jay in Front**, 2026-09-23 ~02:25 UTC. The Front API has no tag-archive field (`PATCH {"is_archived": true}` returns 200 but is silently ignored), and a GET doesn't expose the status either. The evidence is `updated_at`: all 647 cluster tags, and no other tags, changed at 02:24–02:25 UTC.

**Superseded — was:** deleting the 47 junk `cluster/*` tags failed; the Front API token lacks the `tags:delete` scope (403 on every one, nothing changed). All 47 were verified to be used only on automated `notifications@cfmins.org` mail plus one archived marketing email. Delete them in Front → Settings → Tags, or add `tags:delete` to the token. Inventory: session scratchpad `cluster_tags.json`. Separately, `m4_cluster` inventing a new cluster name per run is its own follow-up. Also, `cfm-daily-jobs-sa` can't `run.jobs.run` on `cfm-front-mail`, so a pipeline run can't be triggered from this PC; verify fixes on the next scheduled run.

## Next Steps

0. ~~Review this session's code changes with Jay before committing/deploying~~ — done; Jay said "deploy" and it shipped, commit `722f1ce`, verified live.
1. **Test workbook end-to-end** — Refresh Triage (verify all columns/colors, the new Email column), Save Actions (mark one item done, try a `delegate to admin`/`send to sally` action, confirm Firestore update + Front archive), Send Briefing (confirm email arrives).
